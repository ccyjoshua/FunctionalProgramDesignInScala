...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map((b: Block, m: Move) => (b, m :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter((b: Block, _) => !explored.contains(b))
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match
      ...
      case (b, ms) #:: tail =>
        ...
        (b, ms) #:: from(tail ++ newNeighbors, newExplored)
//        newNeighbors ++ from(tail ++ newNeighbors, newExplored)
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    pathsFromStart.filter((b, _) => b.b1 == goal && b.b2 == goal)
